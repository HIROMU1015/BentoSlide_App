...
After plan approval, the App's optional AI HTML Initial Generation route is limited to schema v2 `single`/`imported`, `whole_deck` projects in `html_authoring` with complete approved sections and no canonical HTML/registry, active AI job, or unresolved candidate. One isolated `.bento-ai/runs/<job>/` workspace receives an immutable snapshot of the approved planning contract, request/project context, manifest-authorized primary/evidence/supplementary sources, and required specifications. The SDK may write only the workspace candidate, has no network or interactive approval, and may not generate images or active content. Candidate registration requires exact section/slide membership and order, source/registry/provenance closure, safe dependencies, factual grounding, unchanged inputs, and existing browser validation evidence for every slide. Generation never changes canonical HTML/registry or workflow state. Explicit apply uses a process-local token and delegates to `command_apply_initial_html_candidate`, which revalidates every bound context, candidate, dependency, and browser-evidence revision under one union `WriterLease`, then atomically installs the HTML/registry, opens the existing whole-deck `html_review`, updates the work log, and marks the candidate applied. Stale/tampered candidates, concurrent writers, and transaction failures leave canonical artifacts and state unchanged. Cancel only closes the candidate; an interrupted running job recovers as a retryable failure.

The generation snapshot itself also holds one union `WriterLease` across `deck.yaml`, planning/request/work-log paths, the source manifest and every declared source, plus the required specifications. Each input is read once under that lease; the workspace copy, revision binding, and planning/context signatures are all derived from those exact bytes. A supported concurrent writer therefore either completes before the snapshot or is rejected while the snapshot is held; generation must never combine pre-update workspace content with post-update revision metadata.
